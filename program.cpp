...
void doMove(int newX, int newY){
	if (grid[newX][newY][targetZpos]==-1 && grid[newX][newY][targetZpos-1]!=-1){
		targetXpos = newX;
		targetYpos = newY;
	}
	...
	else if (grid[newX][newY][targetZpos]==-1 && grid[newX][newY][targetZpos-1]==-1){
		if (targetZpos-1==0 || grid[newX][newY][targetZpos-2]!=-1){
			targetXpos = newX;
			targetYpos = newY;
			targetZpos = targetZpos - 1;
		}else{
	...
	if (gameOver == false){
		...
		if (KEY_E){
			if (xpos==targetXpos && ypos==targetYpos && zpos==targetZpos && remainingFalls == 0){
				...
				for (z=0; z<N; z++){
					if (x==round((N-1)/2.0) && y==round((N-1)/2.0) && z==0){
						continue;
					}
					if (grid[x][y][z] != -1){
						grid[x][y][z] = -1;
